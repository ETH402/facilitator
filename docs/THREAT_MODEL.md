...
|---|---|---|---|---|---|
| Gas draining | facilitator ETH; malicious buyer floods valid/reverting authorizations | ETH loss | verification simulates before a payment record exists, fee caps, bounded hot balance, calldata allowlist | gas/revert alerts | `/settle` neither re-simulates nor re-reads `authorizationState`, so a nonce consumed after `/verify` buys a revert; per-merchant quotas are not implemented |
...
| Gas draining via valid authorization | facilitator ETH; attacker settles a genuine self-to-self payment | unbounded gas loss | `/settle` admits only payments whose recipient is an active registered merchant; suspension revokes it | per-merchant spend dashboards | per-merchant quotas are **not implemented**, so an admitted merchant is bounded only by the shared per-client rate limit; registration is not Sybil-resistant |
